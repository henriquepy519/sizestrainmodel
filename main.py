...
def export_experimental_file():
    if request.method == 'POST':
        ...
        if file_format == 'txt':
            ...
            np.savetxt(file_name, np.c_[Lista1, Lista2])
        elif file_format == 'csv':
            file_name = "experimental_data.csv"
            np.savetxt(file_name, np.c_[Lista1, Lista2], delimiter=',')
    ...
    if uploaded_file.filename != '':
        ...
        if modelo_selecionado == "WHP":
            ...
            for i in range(len(TTH)):
                ...
                if FWHM[i] > 0:
                    Sch1 = 0.154 / (FWHM[i] * np.pi / 180) / np.cos(TTH[i] * np.pi / 360)
                    WHP_x1 = 4 * np.sin(TTH[i] * np.pi / 360) / 0.154
                    WHP_y1 = 1 / (Sch1)
                    Sch.append(Sch1)
                    WHP_x.append(WHP_x1)
                    WHP_y.append(WHP_y1)
                    ...
                    Sch1 = 0.154 / (FWHM1[i] * np.pi / 180) / np.cos(TTH[i] * np.pi / 360)
                    WHP_x1 = 4 * np.sin(TTH[i] * np.pi / 360) / 0.154
                    WHP_y1 = 1 / (Sch1)
                    Sch.append(Sch1)
                    WHP_x.append(WHP_x1)
        ...
        if modelo_selecionado == "SSP":
            for i in range(len(TTH)):
                ...
                if FWHM[i] > 0:
                    d1 = 0.154 / 2 / np.sin(TTH[i] * np.pi / 360)
                    SSP_x1 = d1 ** 2
                    SSP_y1 = (d1 * (FWHM[i] * np.pi / 180) * np.cos(TTH[i] * np.pi / 360) / 0.154) ** 2
                    SSP_x.append(SSP_x1)
                    SSP_y.append(SSP_y1)
                    ...
                    d1 = 0.154 / 2 / np.sin(TTH[i] * np.pi / 360)
                    SSP_x1 = d1 ** 2
                    SSP_y1 = (d1 * (FWHM1[i] * np.pi / 180) * np.cos(TTH[i] * np.pi / 360) / 0.154) ** 2
                    SSP_x.append(SSP_x1)
                    SSP_y.append(SSP_y1)
            ...
            Dm_SSP1 = (k) / np.sqrt(p2[0])  # Tamanho
        ...
        if modelo_selecionado == "UDSM":
            for i in range(len(TTH)):
                ...
                if s_11 != 0 and s_12 != 0 and s_44 != 0:
                    ...
                    # print(1 / E_hkl1)
                    UDSM_x1 = 4 * np.sin(TTH[i] * np.pi / 360) / Ehkl[i] / 0.154
                    UDSM_y1 = (FWHM2[i] * np.pi / 180) * np.cos(TTH[i] * np.pi / 360) / 0.154
        ...
        if modelo_selecionado == "UDEDM":
            for i in range(len(TTH)):
                ...
                if s_11 != 0 and s_12 != 0 and s_44 != 0:
                    ...
                    Ehkl.append(1 / E_hkl1)
                    # print(1 / E_hkl1)
                    UDEDM_x1 = 5.657 * np.sin(TTH[i] * np.pi / 360) / np.sqrt(1 / E_hkl1) / 0.154
                    UDEDM_y1 = (FWHM2[i] * np.pi / 180) * np.cos(TTH[i] * np.pi / 360) / 0.154
            ...
            # Dados e ajuste
            trace1 = go.Scatter(x=UDEDM_x_filtered, y=UDEDM_y_filtered, mode='markers', name='HWP - UDEDM ')
        ...
        if modelo_selecionado == "UDSM - SSP":
            for i in range(len(TTH)):
                ...
                if s_11 != 0 and s_12 != 0 and s_44 != 0:
                    ...
                    d1 = 0.154 / 2 / np.sin(TTH[i] * np.pi / 360)
                    SSP_UDSM_x1 = d1 ** 2
                    SSP_UDSM_y1 = (d1 * (FWHM2[i] * np.pi / 180) * np.cos(TTH[i] * np.pi / 360) / 0.154) ** 2
